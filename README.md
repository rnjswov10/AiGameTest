...
- Main menu buttons: `로컬 1v1`, `Steam 로비`, `설정`, and `종료`.
- The `Steam 로비` screen includes `Steam 연결`, `로비 만들기`, `공개 로비 찾기`, `붙여넣기`, `참가`, `+ 초대`, `코드 복사`, `준비 완료`, and `로비 나가기`.
- Steam lobby creation, search, and join attempts show loading and failure screens.
...
- Player actions use on-screen `Summon`, `Merge`, `Attack`, and `Boss` buttons.
- `Restart` restarts the match.
- Steam lobby controls are available as mouse-click buttons.
- Online matches wait in the Steam lobby until both players press `준비 완료`.
- In Steam online mode, host is Player A and client is Player B. Each side uses the player action buttons for their own board.
